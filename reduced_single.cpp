...
public:
    ...
    circuit(const vector<UINT> &P, const vector<int> &D, const int &init_target_number) : point(P), depth(D)
    {
        gate_number = 0;
        target_number = init_target_number;
        Nu = 0;
    }
...
int parse_function(string str, vector<UINT> &X, vector<UINT> &Y)
{
    ...
    if (str.find(' ') != string::npos) // str is bit sliced representation
    {
        ...
        {
            UINT y = (UINT)stoul(temp, nullptr, 16);
            if (find(Y.begin(), Y.end(), y) == Y.end() && y != 0 && y != (UINT)(-1)) // exclude the repeated points, the constant point 0 and 1, especially when the S-box is irreversible
                Y.push_back(y);
            ++bit_number;
        }
    }
    else // str is LUT representation
    {
        ...
        for (int i = 0; i < bit_number; ++i)
        {
            ...
            if (find(Y.begin(), Y.end(), y) == Y.end()) // exclude the repeated points, especially when the S-box is irreversible
                Y.push_back(y);
        }
    }
    for (int i = 0; i < bit_number; ++i)
...
{
    int cost_key = step;
    int Delta = 0; // a deviation in which the target_number of a useful circuit is smaller than the max_target_number, usually Delta = 0 or 1
    ...
    while (cost_key <= max_number * opt_cost[*(--opt_name.end())])
    {
        ...
        for (string Gate : opt_name)
        {
            ...
            if (cost_dic_iter != cost_dic.end())
                ...
                {
                    // prejudge if current circuit and next circuits will satisfy the request of target number
                    if (element.target_number + 1 < max_target_number - Delta)
                        break;
                    ...
                    for (vector<UINT>::size_type i = 0; i < temp_point.size(); ++i)
                    {
                        ...
                        // judge if the new point is valid, and judge if the circuit with this point satisfies the request of target number
                        if (!(element.repeated_point(new_point) || ((new_depth == max_depth && !is_target) || (new_depth < max_depth && !is_target && new_depth + min_depth > max_depth)) || element.target_number + is_target < max_target_number - Delta))
                        {
                            circuit new_circuit;
                            ...
                            if (!(new_circuit.unpromising_circuit(max_number, bit_number) || repeated_vector_point(new_circuit.gate_number, sorted_point)))
                            {
                                ...
                                else
                                {
                                    ...
                                    if (new_circuit.target_number > max_target_number)
                                        max_target_number = new_circuit.target_number; 
                                }
                            }
                        }
                    }
        ...
        if (!cost_value_list.empty())
        {
            ...
            auto iter = cost_value_list.begin();
            while (iter != cost_value_list.end() && iter->target_number >= max_target_number - Delta)
                ++iter;
            vector<circuit> cost_value_vector(cost_value_list.begin(), iter);
...
int main(int argc, char *argv[])
{
    ...
    // get all possible gates according to conf_name, and get the step
    int step = extract_gate(area_conf_name, depth_conf_name);

    // preprocess the set of target point, especially when the S-box is irreversible
    int init_target_number = bit_number - Y.size();
    for (auto ele : Y)
        if (find(X.begin(), X.end(), ele) != X.end() || ele == 0 || ele == (UINT)(-1)) // when a target point is one of the input points, or equals to constant o or 1, it is believed to be reached
            ++init_target_number;
    circuit start(X, vector<int>(bit_number, 0), init_target_number);
